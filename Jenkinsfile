       ...
       stage('Prepare environment') {
        ...
        stage('Integration tests') {
            steps {
                ...
                sh "./mvnw failsafe:integration-test failsafe:verify -DargLine=-Dtest.selenium.hub.url=http://$SELENIUM_GRID_HOST:$SELENIUM_GRID_PORT -Dtest.target.server.url=http://20.1.0.118:5555" + "$APP_CONTEXT_ROOT/".replace('//', '/')
