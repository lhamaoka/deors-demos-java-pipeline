       ...
       stage('Prepare environment') {
        ...
        stage('Integration tests') {
            steps {
                ...
                sh "./mvnw failsafe:integration-test failsafe:verify -DargLine=-Dtest.selenium.hub.url=http://$SELENIUM_GRID_HOST:$SELENIUM_GRID_PORT -Dtest.target.server.url=http://selenium-edge-node:6900" + "$APP_CONTEXT_ROOT/".replace('//', '/')
